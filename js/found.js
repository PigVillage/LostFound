...
//设置moment全局中文
moment.locale('zh-cn');
var LostInfo = Bmob.Object.extend("found_info");
...
query.find({
    success: function(results) {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            var lost_time = object.get('found_time');
                ...
                .prepend($('<li class="container-fluid">')
                    .append($('<div class="jumbotron" style="background-color: rgba(135,206,250,0.3)">' )
                        .append($('<p class="alert alert-danger">').text('物品描述:' + info),
                            ...
                            $('<p class="btn btn-primary" style="margin-top: 3%">').text('发现地点:' + location),
                            $('<br>'),
                            $('<p class="btn btn-info">').text('发现时间:' + lost_time),
                            $('<br>'),
                            $('<p class="btn btn-success">').text('好心人:' + people),
                            $('<br>'),
